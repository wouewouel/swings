...
using namespace std;
    ...
    bool Vecteur::dimension_ok(Vecteur const& autre) const { // On teste ici si les dimensions sont bonnes
            if(coordonnees.size() == (autre.getcoord()).size() ){
                ...
                throw Err_dim;
                //on ne return pas false car l'execution ne reprends jamais apres le throw
                }
            }

    bool Vecteur::dimension_3(const Vecteur &autre) const { //on teste ici si les deux vecteurs sont de dimensions 3
